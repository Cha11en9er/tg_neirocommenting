...
VERBOSE_COMMENT_MODE = getattr(cfg, "VERBOSE_COMMENT_MODE", False)


def _test_skip_reason(candidate: PostCandidate, channel_key: str) -> str | None:
    if state.is_processed(channel_key, candidate.message_id):
        return "уже обработан"
    if not candidate.comments_open:
        return "комментарии закрыты"
...
def _build_verbose_comment(
    ...
    lines = [
        ...
        "",
        f"Пост #{post.message_id}",
        f"Возраст: {_format_timedelta(post.age)}",
        f"Комментариев под постом: {post.comment_count}",
        "",
        f"Тип (LLM): {post_type}",
        f"Подходит: {'да' if suitable else 'нет'}",
        f"Проверка LLM: {classify_reason}",
    ]
    if prod_filter:
        lines.extend(["", f"В prod пропустили бы: {prod_filter}"])
    else:
        lines.extend(["", "Фильтры prod: ок"])

    preview = post.text.strip().replace("\n", " ")
    if len(preview) > 400:
        preview = preview[:400] + "…"
    lines.extend(["", "Текст поста:", preview or "—"])

    if generated:
        lines.extend(["", "Сгенерированный комментарий:", generated])
    elif suitable:
        lines.extend(["", "Сгенерированный комментарий: не удалось создать"])
...
async def process_channel_verbose(ch: ChannelConfig) -> bool:
    """Тест: весь разбор LLM — в комментарий под постом, без фильтра по комментариям."""
    key = _channel_key(ch.channel)
    ...
    for post in candidates:
        reason = _test_skip_reason(post, key)
        if reason:
            if reason == "комментарии закрыты":
                state.mark_rejected(key, post.message_id)
            print(f"  ↳ пост {post.message_id}: пропуск — {reason}")
            continue

        prod_filter = _prod_filter_reason(post, key, ch)
        print(
            f"\n🔍 @{ch.channel}: пост {post.message_id} — "
            f"тестовый разбор в комментарий"
        )

        suitable, classify_reason, post_type = await classify_post(post.text)
        generated = await generate_comment(post.text) if suitable else None

        comment = _build_verbose_comment(
            post,
            suitable,
            classify_reason,
            post_type,
        ...
        try:
            ...
            print(
                f"✅ @{ch.channel}: тестовый комментарий к посту {post.message_id}"
            )
            return True
        except Exception as e:
...
async def process_channel(ch: ChannelConfig) -> bool:
    ...
    if VERBOSE_COMMENT_MODE:
        return await process_channel_verbose(ch)
